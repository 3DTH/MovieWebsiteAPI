    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-b from-gray-900 to-black">
        <div className="bg-gray-800/50 backdrop-blur-md p-8 rounded-xl max-w-md text-center">
          ...
          <p className="text-gray-300 mb-6">Vui lòng quay lại trang phim để xem thông tin chi tiết hoặc thử lại sau.</p>
          <Link href={`/movies/${movieId}`} className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors inline-flex items-center gap-2">
            <FiArrowLeft />
            Quay lại trang phim
          </Link>
        </div>
            ...
            <div className="absolute top-0 left-0 right-0 z-40 pointer-events-none">
              <div className="bg-gradient-to-b from-black/80 to-transparent p-4 flex items-center justify-between">
                <div className="flex items-center gap-3 pointer-events-auto">
                  ...
                  <h2 className="text-white font-medium text-lg hidden md:block">{movie.title}</h2>
                </div>
                <div className="flex items-center gap-2 pointer-events-auto">
                  <button
                    onClick={toggleFavorite}
                    className={`${isFavorite ? 'bg-red-600' : 'bg-black/40'} backdrop-blur-sm hover:bg-red-600 text-white p-2 rounded-full transition-all duration-300`}
                  >
                    <FiHeart size={20} />
                  </button>
                  <button
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center bg-yellow-500/20 text-yellow-500 px-4 py-2 rounded-lg">
                  ...
                  <span className="font-bold mr-1">{movie.voteAverage.toFixed(1)}</span>
                  <span>/10</span>
                </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/30 rounded-xl p-6 backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {movie.cast.slice(0, 6).map((castMember, index) => (
                      <div key={index} className="flex items-center gap-3 bg-gray-800/50 p-3 rounded-lg hover:bg-gray-700/50 transition-colors">
                        ...
                        <div>
                          <p className="text-white font-medium text-sm">{castMember.actor.name}</p>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <svg 
                                key={star} 
                                className={`w-5 h-5 ${star <= Math.round(movie.voteAverage / 2) ? 'text-yellow-500' : 'text-gray-500'}`} 
                                fill="currentColor" 
                                viewBox="0 0 20 20"
                              >
                          ...
                          <span className="text-white">{Math.round(movie.voteAverage / 2)}/5</span>
                        </div>
                        <div className="w-full bg-gray-700 rounded-full h-2.5">
                          <div className="bg-yellow-500 h-2.5 rounded-full" style={{ width: `${(movie.voteAverage / 10) * 100}%` }}></div>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <svg 
                            key={star} 
                            className={`w-5 h-5 ${star <= 4 ? 'text-yellow-500' : 'text-gray-500'}`} 
                            fill="currentColor" 
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <svg 
                            key={star} 
                            className={`w-5 h-5 ${star <= 5 ? 'text-yellow-500' : 'text-gray-500'}`} 
                            fill="currentColor" 
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-800/30 rounded-xl p-6">
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="text-gray-400 text-sm mb-1">Ngày phát hành</h4>
          ...
          <div className="mt-12 border-t border-gray-800 pt-8">
            ...
            <div className="bg-gray-800/30 rounded-xl p-6 mb-6 backdrop-blur-sm">
              <textarea 
                ...
                className="w-full bg-gray-700/50 border border-gray-600 rounded-lg p-4 text-white resize-none focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all"
                rows={3}
              ></textarea>
            ...
            <div className="space-y-6">
              {/* Bình luận mẫu */}
              {[1, 2, 3].map((_, index) => (
                <div key={index} className="bg-gray-800/30 rounded-xl p-6 backdrop-blur-sm">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium text-white">Người dùng ẩn danh</h4>
                      ...
                      <p className="text-gray-300">
                        {index === 0 
                          ? "Phim hay quá! Cảnh quay đẹp, diễn xuất tuyệt vời." 
                          : index === 1 
                            ? "Tôi thích cách đạo diễn xây dựng nhân vật chính, rất sâu sắc và chân thực." 
                            : "Âm thanh và hình ảnh chất lượng cao, trải nghiệm xem phim tuyệt vời!"}
                      </p>
                      <div className="flex items-center gap-4 mt-3 text-sm">
                        <button className="flex items-center gap-1 text-gray-400 hover:text-red-500 transition-colors">
                          ...
                          Thích ({Math.floor(Math.random() * 50)})
                        </button>
              ...
              <div className="text-center mt-8">
                <button className="px-6 py-2 bg-gray-800/50 hover:bg-gray-700/50 text-white rounded-md transition-colors">
                  Xem thêm bình luận
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="text-center md:text-left">
                ...
                <p className="text-gray-400 text-sm">
                  © {new Date().getFullYear()} Bản quyền thuộc về nhà phát hành phim. Trang web chỉ cung cấp đường dẫn xem phim.
                </p>
              </div>
              
              <div className="flex flex-wrap gap-4 justify-center">
                <Link href={`/movies/${movieId}`} className="text-gray-300 hover:text-white transition-colors text-sm">
                  Thông tin phim
                </Link>
