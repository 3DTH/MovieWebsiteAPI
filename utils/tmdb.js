...
const tmdbApi = {
  ...
  getPopularMovies: async (page = 1) => {
    try {
      ...
      if (response.data.results) {
        console.log(`Lấy thành công ${response.data.results.length} phim từ trang ${page}`);
  ...
  getNowPlaying: async (page = 1) => {
    try {
      ...
      console.log(`Lấy thành công ${response.data.results.length} phim đang chiếu từ trang ${page}`);
      return response.data;
    } catch (error) {
      console.error("Lỗi chi tiết:", error.response ? error.response.data : error.message);
